...
LorentzVec VertexHelper::GetSV4Vector(const reco::Vertex &vertex) {

  LorentzVec vertex4Vec;

  for(const auto &trackRef : vertex.refittedTracks()) {
    const double pt(trackRef.pt());
    const double eta(trackRef.eta());
    const double phi(trackRef.phi());
    const double mass(0.);
...
double VertexHelper::GetTrackWeight(const reco::Vertex &vertex, const reco::Track &track) {

  if(!isInVertex(vertex, track)) {
    std::cout << "track is not in this vertex!" << std::endl;
    return -1.;
  } 
  
  double weight(-1.);
  for(const auto &refTrack : vertex.tracks()) {
    if(refTrack->pt() == track.pt() && refTrack->eta() == track.eta())
      weight = vertex.trackWeight(refTrack);
  }
  return weight;
}
